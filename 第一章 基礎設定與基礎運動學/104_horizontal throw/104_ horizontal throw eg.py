...
ball = sphere(radius = radius, pos = vec(-floor.size.x*0.5,49,0), color = color.red, make_trail=True, trail_color = color.white, v = v, a = a)
...
vtplot = graph(title = "v-t plot", xtitle = "t(s)", ytitle = "black:vx(m/s) ; blue:vy(m/s)", height = 400, width = 600)

xy = gcurve(graph = xyplot, color = color.red)
vxt = gcurve(graph = vtplot, color = color.black)
vyt = gcurve(graph = vtplot, color = color.blue)
